...
local LootSeed = LootItem.Extend("LootSeed")

function LootSeed:Loot()
    if not self.player then
        return false
    end
    local reason = "PetLootSeedSkill"
    ...
    if chance then
        if self.skillEventData.items then
            ...
            PetKismet:PetLootSeed(self.petActor, self.player, chance / denominator)
        end
    end
end
