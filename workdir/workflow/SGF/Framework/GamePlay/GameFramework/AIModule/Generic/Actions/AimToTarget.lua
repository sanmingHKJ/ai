        ...
        if target then
            ...
            if rotating and rotating > 0 then
                if runtime.owner.rotating then
                    ...
                    --设置瞄准方向
                    local targetPos = target:GetPosition() + Vec3.New(0, 100, 0)
                    local selfPos = runtime.owner:GetPosition()
                    local aimDir = (targetPos - selfPos):Normalized()
                    runtime.owner.AvatarComponent:SetAimDirection(aimDir)
                    runtime.owner.AvatarComponent:SetAimPosition(targetPos)
