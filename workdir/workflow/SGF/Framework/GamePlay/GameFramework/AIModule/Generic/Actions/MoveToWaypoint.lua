...
local M = { 
    ...
    Run = function(node, runtime, args)
        ...
        if moveTimeOut ~= nil then
            ...
            local targetLocation = runtime:GetBB(node, args.targetLocationBBKey)
            ...
            local targetWaypoint = runtime:GetBB(node, args.targetWaypointBBKey)
            if targetWaypoint == nil then
                return EResult.FAIL
            end

            local targetLocation = targetWaypoint:GetRandomPosition()
            local pos = Vec3.New(targetLocation[1],targetLocation[2],targetLocation[3])
