...
local UIResourceGroup = UIClass.New("UIResourceGroup")
...
--构造函数
function UIResourceGroup:Constructor(resType)
    self.resType = resType
    self.resources = {}
end

--加载资源
function UIResourceGroup:LoadResource(name, callback)
...
--卸载资源，不卸载，仅减少引用计数
function UIResourceGroup:UnloadResource(name)
...
--强制卸载资源
function UIResourceGroup:ForceUnloadResource(name)
    local res = self.resources[name]
    if res then
...
--清理长时间未使用的资源
function UIResourceGroup:ClearUnusedResources(timeOut)
