...
--开发分辨率
UIUtils.DESIGN_RESOLUTION = Vec2.New(1280, 720)
--资源分辨率
UIUtils.RESOURCE_RESOLUTION = Vec2.New(1920, 1080)

--获取缩放比例
function UIUtils:GetDesignScale()
    local scaleX = UIUtils.DESIGN_RESOLUTION.x / UIUtils.RESOURCE_RESOLUTION.x
    local scaleY = UIUtils.DESIGN_RESOLUTION.y / UIUtils.RESOURCE_RESOLUTION.y
    return Vec2.New(scaleX, scaleY)
end

--获取视口分辨率
function UIUtils:GetViewportSize()
    local resolution = game:GetService("WorldService"):GetUISize()
    return Vec2.New(resolution.X, resolution.Y)
end

--获取视口矩形
function UIUtils:GetViewportRect()
    local viewportSize = self:GetViewportSize()
    return Rect.New(0, 0, viewportSize.x, viewportSize.y)
end

--规范化精灵路径
function UIUtils:FullSpritePath(sprite)
...
--获取相对路径
function UIUtils:RelativeSpritePath(sprite)
...
--调整某个控件所在父节点中的排序位置
function UIUtils:SetSiblingIndex(control, index)
    local parent = control.Parent
    if not parent then
        UILog:Error("UIUtils:SetSiblingIndex: parent is nil")
        return
    end
    local children = parent.Children
    if not children then
        UILog:Error("UIUtils:SetSiblingIndex: children is nil")
        return
    end
...
function UIUtils:GetSiblingIndex(control)
    local parent = control.Parent
    if not parent then
        UILog:Error("UIUtils:GetSiblingIndex: parent is nil")
        return
    end
    local children = parent.Children
    if not children then
        UILog:Error("UIUtils:GetSiblingIndex: children is nil")
        return
    end
    for i, child in ipairs(children) do
...
function UIUtils:SetParent(control, parent, keepScreenPos)
    if not control then
    ...
    if keepScreenPos then
        screenPos = UIUtils:GetScreenPosition(control)
    end
    control.Parent = parent
    if keepScreenPos then
        UIUtils:SetScreenPosition(control, screenPos)
    end
end

--排序渲染索引
function UIUtils:SortRenderIndex(control)
...
--设置到最上层
function UIUtils:SetToTop(control)
    local parent = control.Parent
    if not parent then
        UILog:Error("UIUtils:SetToTop: parent is nil")
        return
    end
    local children = parent.Children
    if not children then
        UILog:Error("UIUtils:SetToTop: children is nil")
        return
    end
    UIUtils:SetSiblingIndex(control, #children)
end

--设置到最下层
function UIUtils:SetToBottom(control)
    local parent = control.Parent
    if not parent then
        UILog:Error("UIUtils:SetToBottom: parent is nil")
        return
    end
    local children = parent.Children
    if not children then
        UILog:Error("UIUtils:SetToBottom: children is nil")
        return
    end
    UIUtils:SetSiblingIndex(control, 1)
end

--设置颜色
function UIUtils:SetColorInHierarchy(control, color, recursive)
    if not control then
...
--设置透明度
function UIUtils:SetAlphaInHierarchy(control, alpha, recursive)
    if not control then
...
--设置颜色
function UIUtils:SetColor(control, color)
    if not control then
...
--获取颜色
function UIUtils:GetColor(control)
    if not control then
...
--设置透明度
function UIUtils:SetAlpha(control, alpha)
    if not control then
    ...
    if control.Alpha then
        if control.ClassType == "UIImage" then
            control.Alpha = alpha
        else
            control.Alpha = a
        end
...
--获取透明度
function UIUtils:GetAlpha(control)
    if not control then
...
--查找子控件
function UIUtils:FindChildControl(control, controlName, recursive)
    recursive = recursive or true
...
--获取屏幕缩放
function UIUtils:GetScreenScale(control)
    local scaleX = 1
    local scaleY = 1
...
function UIUtils:AdjustLayoutChildren(control, children,
...
--水平布局
function UIUtils:AdjustHLayoutChildren(control, children,
    layoutOrigin, 
...
--纵向布局
function UIUtils:AdjustVLayoutChildren(control, children,
    layoutOrigin, 
    layoutReverse, layoutPadding, layoutSpacing, layoutCallback)
    local layoutRect = self:GetScreenRect(control)
    local layoutItems = {}
...
--@param origin 对齐方式
function UIUtils:HLayoutChildren(control, spacingX, paddingX, origin)
...
--@param paddingX 水平内边距
--@param origin 对齐方式
function UIUtils:HLayout(controls, spacingX, paddingX, origin)
    if #controls == 0 then
        return
    end
    spacingX = spacingX or 0
...
--@param origin 对齐方式
function UIUtils:VLayoutChildren(control, spacingY, paddingY, origin)
...
--@param paddingY 垂直内边距
--@param origin 对齐方式
function UIUtils:VLayout(controls, spacingY, paddingY, origin)
    if #controls == 0 then
        return
    end
    spacingY = spacingY or 0
...
--获取屏幕位置
function UIUtils:GetScreenPosition(control)
...
--设置屏幕位置
function UIUtils:SetScreenPosition(control, pos)
    local currentControl = control
...
--获取父节点屏幕矩形
function UIUtils:GetParentScreenRect(control)
    local parent = control.Parent
    if not parent then
...
--获取屏幕矩形
function UIUtils:GetScreenRect(control)
    local pos = control.Position
    local size = control.Size
    local pivot = control.Pivot
...
--设置屏幕矩形
function UIUtils:SetScreenRect(control, rect)
    local currentControl = control
    local parentTransforms = {}
...
--获取屏幕位置X
function UIUtils:GetScreenPositionX(control)
    local pos = self:GetScreenPosition(control)
    return pos.x
end

--设置屏幕位置X
function UIUtils:SetScreenPositionX(control, x)
    local pos = self:GetScreenPosition(control)
    pos.x = x
...
--获取屏幕位置Y
function UIUtils:GetScreenPositionY(control)
    local pos = self:GetScreenPosition(control)
    return pos.y
end

--设置屏幕位置Y
function UIUtils:SetScreenPositionY(control, y)
    local pos = self:GetScreenPosition(control)
    pos.y = y
    self:SetScreenPosition(control, pos)
end
...
--获取父节点屏幕尺寸
function UIUtils:GetParentScreenSize(control)
    local parent = control.Parent
    if not parent then
...
--获取屏幕尺寸
function UIUtils:GetScreenSize(control)
    local size = control.Size
...
--设置屏幕尺寸
function UIUtils:SetScreenSize(control, size)
    local currentControl = control
    local parentScales = {}
...
--获取屏幕宽度
function UIUtils:GetScreenWidth(control)
    return self:GetScreenSize(control).x
end

--设置屏幕宽度
function UIUtils:SetScreenWidth(control, width)
...
--获取屏幕高度
function UIUtils:GetScreenHeight(control)
    return self:GetScreenSize(control).y
end

--设置屏幕高度
function UIUtils:SetScreenHeight(control, height)
    local size = self:GetScreenSize(control)
    size.y = height
    self:SetScreenSize(control, size)
end
...
--设置全屏
function UIUtils:SetFullScreenSize(control)
    local viewportSize = self:GetViewportSize()
    local pivot = control.Pivot
...
--设置拉伸X
function UIUtils:SetStretchX(control, stretchX)
    if not control then
...
--获取拉伸X 
function UIUtils:GetStretchX(control)
    if not control then
...
--设置拉伸Y
function UIUtils:SetStretchY(control, stretchY)
    if not control then
...
--获取拉伸Y
function UIUtils:GetStretchY(control)
    if not control then
...
--设置拉伸
function UIUtils:SetStretch(control, stretchX, stretchY)
    if not control then
...
--获取拉伸
function UIUtils:GetStretch(control)
    if not control then
...
--设置锚点X
function UIUtils:SetAnchorX(control, anchorX)
    if not control then
...
--获取锚点X
function UIUtils:GetAnchorX(control)
    if not control then
...
--设置锚点Y
function UIUtils:SetAnchorY(control, anchorY)
    if not control then
...
--获取锚点Y
function UIUtils:GetAnchorY(control)   
    if not control then
...
--设置锚点
function UIUtils:SetAnchor(control, anchorX, anchorY)
    if not control then
...
--获取锚点
function UIUtils:GetAnchor(control)
    if not control then
...
--设置中间X
function UIUtils:CenterX(control)
    local parentScreenRect = self:GetParentScreenRect(control)
...
--设置中间Y
function UIUtils:CenterY(control)
    local parentScreenRect = self:GetParentScreenRect(control)
...
--设置中间
function UIUtils:Center(control)
    local parentScreenRect = self:GetParentScreenRect(control)
    local centerX = parentScreenRect.width / 2
    local centerY = parentScreenRect.height / 2
    local pivot = control.Pivot
...
--设置位置
function UIUtils:SetPosition(control, position)
    if not control then
...
function UIUtils:GetPosition(control)
    if not control then
...
--设置左上角
function UIUtils:SetLeftTopCorner(control, leftTopCorner)
    if not control then
...
--获取左上角
function UIUtils:GetLeftTopCorner(control)
    if not control then
...
--设置右上角
function UIUtils:SetRightTopCorner(control, rightTopCorner)
    if not control then
...
--获取右上角
function UIUtils:GetRightTopCorner(control)
    if not control then     
...
--设置左下角
function UIUtils:SetLeftBottomCorner(control, leftBottomCorner)
    if not control then
...
--获取左下角
function UIUtils:GetLeftBottomCorner(control)  
    if not control then
...
--设置右下角
function UIUtils:SetRightBottomCorner(control, rightBottomCorner)
    if not control then
...
--获取右下角
function UIUtils:GetRightBottomCorner(control)
    if not control then
...
--设置大小
function UIUtils:SetSize(control, size)
    if not control then
...
function UIUtils:GetSize(control)
    if not control then
...
--获取宽度
function UIUtils:GetWidth(control)
    if not control then
...
--获取高度
function UIUtils:GetHeight(control)
    if not control then
...
--设置Pivot
function UIUtils:SetPivot(control, pivot)
    if not control then
...
function UIUtils:GetPivot(control)
    if not control then
...
--获取锚点在点上的位置
function UIUtils:GetPivotPoint(control)
    if not control then
...
--设置旋转
function UIUtils:SetRotation(control, rotation)
    if not control then
...
function UIUtils:GetRotation(control)
    if not control then
...
--设置缩放
function UIUtils:SetScale(control, scale)
    if not control then
...
function UIUtils:GetScale(control)
    if not control then
...
--设置可见性
function UIUtils:SetVisible(control, visible)
    if not control then
...
function UIUtils:IsVisible(control)
    if not control then
...
--是否在可见层级中
function UIUtils:IsVisibleInHierarchy(control)
    if not control then
...
function UIUtils:SetRenderIndex(control, renderIndex)
    if not control then
...
function UIUtils:SetFontSize(control, fontSize)
    if not control then
...
function UIUtils:SetBMText(control, text)
    if not control then
...
function UIUtils:SetBMFonts(control, fonts)
    if not control then
...
function UIUtils:SetBMPng(control, png)
    if not control then
...
function UIUtils:CalculateAnchoredRect(control, size, pivot, offset, anchor)
...
--布局起点转换为锚点
function UIUtils:LayoutOriginToAnchor(layoutOrigin)
...
--minWidth: 最小宽度
--minHeight: 最小高度
function UIUtils:FitSize(control, expandX, expandY, minWidth, minHeight, applyWidth, applyHeight)
    applyWidth = applyWidth == nil and true or applyWidth
    applyHeight = applyHeight == nil and true or applyHeight
    ...
    local oldRect = self:GetScreenRect(control)
    if not applyWidth then
        childrenRect.x = oldRect.x
        childrenRect.width = oldRect.width
    end
    if not applyHeight then
        childrenRect.y = oldRect.y
        childrenRect.height = oldRect.height
    end
    ...
    for index, child in ipairs(control.Children) do
        if child.Visible then
            local childRect = childrenRects[rectIndex]
            if childRect then
                UIUtils:SetScreenRect(child, childRect)
            end
...
--调整大小并扩展目标
function UIUtils:ResizeAndExpandTarget(control, size, pivot, targetControl)
    local rect = self:GetScreenRect(control)
...
--调整大小
function UIUtils:Resize(control, size, pivot)
    local rect = self:GetScreenRect(control)
...
--计算子控件矩形
function UIUtils:CalculateChildrenScreenRect(control)
...
--设置锚点预设
function UIUtils:SetAnchorPreset(control, anchorPreset, keepPosition)
...
--计算圆上位置
function UIUtils:CalculateCirclePos(width, height, angle)
...
--将屏幕位置转换为本地位置
function UIUtils:ToLocalPosition(control, screenPos)
    local parent = control.Parent
    if not parent then
        return screenPos
    end
...
--将本地位置转换为屏幕位置
function UIUtils:ToScreenPosition(control, localPos)
    local parent = control.Parent
    if not parent then
        return localPos
    end
...
--计算文本大小
function UIUtils:CalculateTextSize(control, singleLine)
...
function UIUtils:GetNativeSize(control, singleLine)
    singleLine = singleLine or false
    if not control then
...
--设置控件大小为原始大小
function UIUtils:SetNativeSize(control, keepPosition, singleLine)
    singleLine = singleLine or false
    if not control then
...
--设置所有子控件的原始大小
function UIUtils:SetChildrenNativeSize(control, keepPosition, singleLine)
    singleLine = singleLine or false
    if not control then
...
--offset: 偏移
--anchor: 吸附点，0-1之间，x为水平吸附点，y为垂直吸附点，0为左上角，1为右下角
function UIUtils:Align(control, targetControl, pivot, offset, anchor)
...
--停靠在右边
function UIUtils:AlignRight(control, targetControl, spacing)
    ...
    local pos = self:GetScreenPosition(control)
    local size = UIUtils:GetSize(control)
    local anchoredRect = UIUtils:CalculateAnchoredRect(targetControl, size, Vec2.New(0.0, 0.5), Vec2.New(spacing, 0), Vec2.New(1.0, 0.5))
    local newPos = Vec2.New(anchoredRect:GetLeft(), pos.y)
    self:SetScreenPosition(control, newPos) 
end

--停靠在左边
function UIUtils:AlignLeft(control, targetControl, spacing)
    spacing = spacing or 0
    if not targetControl then
        UILog:Error("Align: targetControl is nil")
        return
    ...
    local pos = self:GetScreenPosition(control)
    local size = UIUtils:GetSize(control)
    local anchoredRect = UIUtils:CalculateAnchoredRect(targetControl, size, Vec2.New(1.0, 0.5), Vec2.New(-spacing, 0), Vec2.New(0.0, 0.5))
    local newPos = Vec2.New(anchoredRect:GetLeft(), pos.y)
    self:SetScreenPosition(control, newPos)
end
...
--offset: 偏移
--anchor: 吸附点，0-1之间，x为水平吸附点，y为垂直吸附点，0为左上角，1为右下角
function UIUtils:AlignChain(controls, firstPivot, firstOffset, firstAnchor, otherPivot, otherOffset, otherAnchor)
...
--水平左对齐
function UIUtils:HLAlign(control, targetControl)
    if not targetControl then
...
function UIUtils:HCAlign(control, targetControl)
    if not targetControl then
...
function UIUtils:HRAlign(control, targetControl, pivot, offset, anchor)
    if not targetControl then
...
--纵向顶部对齐
function UIUtils:VTAlign(control, targetControl)
    if not targetControl then
...
--纵向居中对齐
function UIUtils:VCAlign(control, targetControl)   
    if not targetControl then
...
--纵向底部对齐
function UIUtils:VBAlign(control, targetControl)
    if not targetControl then
...
--纵向顶部对齐子控件
function UIUtils:VTAlignChildren(control)
    if not control then
...
--纵向居中对齐子控件
function UIUtils:VCAlignChildren(control)
    if not control then
...
--纵向底部对齐子控件
function UIUtils:VBAlignChildren(control)
    if not control then
...
--加载图片
function UIUtils:LoadImage(control, imagePath, callback)
    if not control then
...
--parentWidth: 父宽度
--parentHeight: 父高度
function UIUtils:CalculateLocalRect(anchoredPosition, sizeDelta, anchorMin, anchorMax, pivot, offsetMin, offsetMax, localScale, 
...
--------------------------------------------Action--------------------------------------------
--运行动作
function UIUtils:RunAction(control, action)
    UIActionRunner:RunAction(control,action)
end
--根据Tag获取动作
function UIUtils:GetActionByTag(control, tag)
    return UIActionRunner:GetActionByTag(control,tag)
end

--根据Tag停止动作
function UIUtils:StopActionByTag(control, tag)
    UIActionRunner:StopActionByTag(control,tag)
end

--根据Tag获取Action的数量
function UIUtils:CountActionByTag(control, tag)
    return UIActionRunner:CountActionByTag(control,tag)
end

--停止所有动作
function UIUtils:StopAllActions(control)
    UIActionRunner:StopAllActions(control)
end

--屏幕坐标转换为本地坐标
function UIUtils:MapScreenToLocal(control, screenPos)
    local mapPos = self:GetScreenPosition(control)
    return screenPos - mapPos
end

--本地坐标转换为屏幕坐标
function UIUtils:MapLocalToScreen(control, localPos)
    local mapPos = self:GetScreenPosition(control)
...
--移除列表中的所有元素
function UIUtils:RemoveAll(t, func)
    local i = 1
    while i <= #t do
        if func(t[i]) then
            table.remove(t, i)
...
--获取列表中符合条件的元素
function UIUtils:FindAll(t, func)
    local result = {}
    for i = 1, #t do
        if func(t[i]) then
...
--获取列表中符合条件的元素索引
function UIUtils:IndexOf(t, value)
    for i = 1, #t do
        if t[i] == value then
            return i
        end
...
--插入元素，去重
function UIUtils:InsertUnique(t, value)
...
--范围随机数            
function UIUtils:Random(min, max)
    return min + math.random() * (max - min)
end

--移动数组元素
function UIUtils:MoveArrayElement(arr, fromIndex, toIndex)
    if fromIndex == toIndex then return arr end
    local element = table.remove(arr, fromIndex)
    if fromIndex > toIndex then
        table.insert(arr, toIndex, element)
    else
        table.insert(arr, toIndex - 1, element)
    end
    return arr
end

--判断字符串是否为空
function UIUtils:IsNullOrEmpty(str)
    return str == nil or str == ""
end

--判断值是否为空或0
function UIUtils:IsNullOrZero(value)
    return value == nil or value == 0
end

--是否客户端
function UIUtils:IsClient()
    return RunService:IsClient()
end
--是否服务端
function UIUtils:IsServer()
    return RunService:IsServer()
end
--是否主机
function UIUtils:IsHost()
    return RunService:IsClient() and RunService:IsServer()
end

--是否仅是客户端
function UIUtils:IsClientOnly()
    return RunService:IsClient() and not RunService:IsServer()
end

--是否仅是服务端
function UIUtils:IsServerOnly()
    return RunService:IsServer() and not RunService:IsClient()
end

function UIUtils:GetNode(path, rootNode)
    --通过.号分割path
...
--获取图标路径
function UIUtils:GetIconPath(icon)
    --将icon的.转换为/
    local iconPath = "sandboxId://UI/" .. icon
    return iconPath
end
...
--@param fitMode 适配模式: "Width"(宽度模式), "Height"(高度模式), "Max"(宽高取最大), "Min"(宽高取最小)
--@return 适配后的宽度、高度、X缩放比例、Y缩放比例
function UIUtils:CalculateImageFitSize(containerWidth, containerHeight, imageWidth, imageHeight, fitMode)
    if not containerWidth or not containerHeight or not imageWidth or not imageHeight then
        UILog:Error("CalculateImageFitSize: 参数不能为空")
        return imageWidth, imageHeight, 1, 1
    end
    
    if containerWidth <= 0 or containerHeight <= 0 or imageWidth <= 0 or imageHeight <= 0 then
        UILog:Error("CalculateImageFitSize: 宽高必须大于0")
        return imageWidth, imageHeight, 1, 1
    end
    
    fitMode = fitMode or "Max"
    
    local scaleX = containerWidth / imageWidth
    local scaleY = containerHeight / imageHeight
    local finalWidth, finalHeight
    
    if fitMode == "Width" then
        -- 宽度模式：以宽度为准，等比例缩放
        finalWidth = containerWidth
        finalHeight = imageHeight * scaleX
        scaleX = scaleX
        scaleY = scaleX 
    elseif fitMode == "Height" then
        -- 高度模式：以高度为准，等比例缩放
        finalWidth = imageWidth * scaleY
        finalHeight = containerHeight
        scaleX = scaleY
        scaleY = scaleY
    elseif fitMode == "Max" then
        -- 宽高取最大：图片完全填充容器，可能超出边界
        local scale = math.max(scaleX, scaleY)
        finalWidth = imageWidth * scale
        finalHeight = imageHeight * scale
        scaleX = scale
        scaleY = scale
    elseif fitMode == "Min" then
        -- 宽高取最小：图片完全适应容器，保持比例，可能有空白
        ...
        UILog:Error("CalculateImageFitSize: 不支持的适配模式: " .. tostring(fitMode))
        return imageWidth, imageHeight, 1, 1
    end
    
    return finalWidth, finalHeight, scaleX, scaleY
end

--随机椭圆位置
function UIUtils:RandomEllipsePos(innerRadiusX, innerRadiusY, outerRadiusX, outerRadiusY)
    -- 参数验证
    if not innerRadiusX or not innerRadiusY or not outerRadiusX or not outerRadiusY then
        UILog:Error("RandomEllipsePos: 参数不能为空")
        return Vec2.New(0, 0)
    end
    
    if innerRadiusX < 0 or innerRadiusY < 0 or outerRadiusX < 0 or outerRadiusY < 0 then
        UILog:Error("RandomEllipsePos: 参数不能为负数")
        return Vec2.New(0, 0)
    end
    
    -- 确保最大半径大于最小半径
    if outerRadiusX <= innerRadiusX then
        outerRadiusX = innerRadiusX + 1
    end
    if outerRadiusY <= innerRadiusY then
        outerRadiusY = innerRadiusY + 1
    end
    ...
    -- 椭圆半径公式: r = (a * b) / sqrt((b * cosθ)² + (a * sinθ)²)
    local innerRadius
    if innerRadiusX == 0 and innerRadiusY == 0 then
        innerRadius = 0
    else
        local denominator = math.sqrt((innerRadiusY * cosAngle) * (innerRadiusY * cosAngle) + 
                                    (innerRadiusX * sinAngle) * (innerRadiusX * sinAngle))
        if denominator > 0 then
            innerRadius = (innerRadiusX * innerRadiusY) / denominator
        else
            innerRadius = 0
        end
    end
    ...
    -- 计算外椭圆在当前角度下的半径
    local outerRadius
    local denominator = math.sqrt((outerRadiusY * cosAngle) * (outerRadiusY * cosAngle) + 
                                (outerRadiusX * sinAngle) * (outerRadiusX * sinAngle))
    if denominator > 0 then
        outerRadius = (outerRadiusX * outerRadiusY) / denominator
    else
        outerRadius = 0
    end
    
    -- 在内外椭圆半径之间生成随机半径
    local radius = innerRadius + randomValue * (outerRadius - innerRadius)
    
    -- 计算最终位置
    local x = radius * cosAngle
    local y = radius * sinAngle
